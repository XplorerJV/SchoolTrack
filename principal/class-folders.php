    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:20px">
        ...
        <div style="background:#fff;border-radius:14px;box-shadow:0 1px 3px rgba(0,0,0,.06);overflow:hidden;border:1px solid #f1f5f9">
            ...
            <div style="padding:18px">
                ...
                <!-- Period status view (read-only for principal) -->
                <div style="font-size:11px;color:#94a3b8;font-weight:600;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px">Period Status</div>
                <div style="display:flex;gap:4px;flex-wrap:wrap;align-items:center;margin-bottom:12px">
                    <?php foreach (PERIOD_TIMES as $p=>$pt):
                        ...
                        $fc   = $done?'#065f46':($part?'#92400e':'#94a3b8');
                    ?>
                    <?php if ($p===4||$p===7): ?><div style="width:1px;height:28px;background:#e2e8f0;margin:0 2px"></div><?php endif; ?>
                    <div title="<?= $pt['label'] ?> (<?= $pt['time'] ?>) — <?= $cnt ?> marked"
                         style="position:relative;display:inline-flex;flex-direction:column;align-items:center;padding:5px 8px;border-radius:6px;font-size:11px;font-weight:700;border:1.5px solid <?= $bc ?>;background:<?= $bg ?>;color:<?= $fc ?>;min-width:34px;text-align:center;cursor:default">
                        P<?= $p ?>
                        <?php if ($done): ?>
